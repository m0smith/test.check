...
(defn ranges-to-gens* 
  "Takes a series of ranges and returns a vector of choose and return generators"
[ranges]
...
(def ranges-to-gens 
  "Memoized ranges-to-gens*"
  (memoize ranges-to-gens*))
...
(def uchar
  "Generates valid Unicode characters (char) from \u0000 to \uFFFF."
  (uchar* uchar?))

(defn- code-point? [^Integer c]
  #?(:clj #(Character/isDefined c) :cljs #(string? c)))


(def code-point 
  "Generates a valid Unicode code point"
  (such-that code-point? 
             (fmap core/int
                   (choices u/all-unicode-ranges))))
...
(def ustring
  " Generates Unicode strings consisting of only single char
  characters. All characters in the string are generated by uchar.  No
  characters beyon \uFFFF are included. May generate unprintable
  characters."
  (fmap clojure.string/join (vector uchar)))

(def ustring-from-code-point
  "Generates Unicode strings consisting of any valid code point. May generate unprintable
  characters."
  (fmap #(String. (int-array %) 0 (count %)) 
        (vector code-point)))


(defn ustring-choices
  "Generate strings in the given range.  See uchar-choices for details
  of ranges. May generate unprintable characters."
  [ranges]
  (fmap clojure.string/join (vector (uchar-choices ranges))))

(def ustring-alphanumeric
  "Generates Unicode alphanumeric strings."
